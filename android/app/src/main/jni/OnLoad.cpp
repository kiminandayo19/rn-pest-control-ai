...
cxxModuleProvider(const std::string &name,
  ...
  // This code register the module so that when the JS side asks for it, the app
  // can return it
  if (name == NativeImageProcessor::kModuleName) {
    return std::make_shared<NativeImageProcessor>(jsInvoker);
  }
...
std::shared_ptr<TurboModule>
javaModuleProvider(const std::string &name,
