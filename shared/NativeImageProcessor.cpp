...
Java_com_pest_1control_1mobile_NativeImageProcessor_initAssetManager(JNIEnv *env, jclass, jobject assetMgr)
{
  /*
...
namespace facebook::react
{
  ...
  void NativeImageProcessor::loadModel(jsi::Runtime &rt)
  {
    try
    {
      ...
      if (!ort_env)
      {
        ort_env = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "onnxruntime");
      }
      ...
      ort_session = std::make_unique<Ort::Session>(*ort_env, model_buffer, model_size, session_options);
  ...
  jsi::Array NativeImageProcessor::predict(jsi::Runtime &rt, const std::string &path, const std::array<int, 4> &dims)
  {
    try
    {
      ...
      int mid_y = dims[2] / 2;
      int mid_x = dims[3] / 2;
      float r_val = channels[0].at<float>(mid_y, mid_x);
      float g_val = channels[1].at<float>(mid_y, mid_x);
      float b_val = channels[2].at<float>(mid_y, mid_x);
      ...
      auto processed_data = processDetections(rt, output_data, output_shape, static_cast<long>(dims_64t[2]), static_cast<long>(dims_64t[3]), 0.25f, 0.4f);

      auto end_time = std::chrono::high_resolution_clock::now();
      auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
      logToJSConsole(rt, std::format("Prediction completed in: {} ms.", std::to_string(duration.count())));
  ...
  void NativeImageProcessor::registerVisionCameraExtension(jsi::Runtime &rt)
  {
    ...
    auto myPlugin = [this](jsi::Runtime &rt, const jsi::Value &thisArgs, const jsi::Value *args, size_t count) -> jsi::Value
    {
      try
      {
        auto _frame = get_frame(rt, args, count);

        [[maybe_unused]] auto width = _frame->get(rt, jsi::PropNameID::forUtf8(rt, "width")).asNumber();
        [[maybe_unused]] auto height = _frame->get(rt, jsi::PropNameID::forUtf8(rt, "height")).asNumber();
        auto pixel_format = _frame->get(rt, jsi::PropNameID::forUtf8(rt, "pixelFormat")).asString(rt).utf8(rt);
        auto get_native_buffer_fn = _frame->get(rt, jsi::PropNameID::forUtf8(rt, "getNativeBuffer")).asObject(rt).asFunction(rt);

        // Get buffer native store it as pointer in uint8
        auto buffer_obj = get_native_buffer_fn.call(rt, jsi::Object::createFromHostObject(rt, _frame)).asObject(rt);
        auto pointer_bigInt = buffer_obj.getProperty(rt, "pointer").asBigInt(rt);
        uintptr_t pointer = pointer_bigInt.getUint64(rt);
        AHardwareBuffer *hardware_buffer = reinterpret_cast<AHardwareBuffer *>(pointer);
        ...
        void *buffer;
        int result = AHardwareBuffer_lock(hardware_buffer, AHARDWAREBUFFER_USAGE_CPU_READ_MASK, -1, nullptr, &buffer);
        if (result != 0)
        {
        ...
        {
          cv::Mat rgba(buffer_description.height, buffer_description.width, CV_8UC4);
        ...
        else if (buffer_description.format == AHARDWAREBUFFER_FORMAT_Y8Cb8Cr8_420)
        {
          cv::Mat gray(buffer_description.height, buffer_description.width, CV_8UC1);
        ...
        auto deleteFn = buffer_obj.getProperty(rt, "delete").asObject(rt).asFunction(rt);
        deleteFn.call(rt);
        ...
        auto processed_data = processDetections(rt, output_data, output_shape, static_cast<long>(height_resize), static_cast<long>(width_resize), 0.7f, 0.7f);
                                  ...
                                  const std::vector<Detection> &detections)
  {
    ...
    for (size_t i = 0; i < detections.size(); i++)
    {
      ...
      obj.setProperty(runtime, "confidence", jsi::Value(detections[i].confidence));
  ...
  {
    auto detections = parseYOLOOutput(output_data, output_shape,
